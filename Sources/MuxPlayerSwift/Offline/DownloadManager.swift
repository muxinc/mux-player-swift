...
actor DownloadManager: NSObject, AVAssetDownloadDelegate {
    ...
    func removeDownload(playbackID: String) async {
        print("[Mux-Offline] removeDownload: called for playbackID \(playbackID)")
    ...
    func handleWaitingForConnectivity(for task: URLSessionTask) {
        guard let playbackID = task.taskDescription else { return }
    ...
    func handleError(for task: URLSessionTask, error: (any Error)?) {
        print("[Mux-Offline] handleError: Error for task with ID \(task.taskIdentifier): \(error?.localizedDescription)")
